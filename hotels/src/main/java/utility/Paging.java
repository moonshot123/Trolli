...
public class Paging {   
   //페이징 관련 변수   
	...
	public String getSearch_p() {
		...
		public void setSearch_p(String search_p) {
			this.search_p = search_p;
		}
		...
		public void setRoompeople(String roompeople) {
			...
			System.out.println("paging의 roompeople : "+roompeople);
		}
		...
		public void setSearchStart(String searchStart) {
			...
			System.out.println("paging의 searchStart : "+searchStart);
		}
		...
		public void setSearchEnd(String searchEnd) {
			...
			System.out.println("paging의 searchEnd : "+searchEnd);
		}
	...
	public Paging(
	      ...
	      this.roompeople = roompeople ;
	      this.sort = sort;
	      if(searchStart.equals("출발하는날짜")) {
	         searchStart="";
	      }
	      this.searchStart = searchStart ;
	      
	      if(searchEnd.equals("돌아오는날짜")) {
	         searchEnd="";
	      }
	      this.searchEnd = searchEnd ;
	   ...
	   private String getPagingHtml( String url ){ //페이징 문자열을 만든다.
	      ...
	      String result = "" ;
	      String added_param = "&search_p=" + search_p + "&roompeople=" + roompeople + "&sort=" + sort 
	            + "&searchStart=" + searchStart + "&searchEnd=" + searchEnd; 
	      
	      if (this.beginPage != 1) { //앞쪽, pageSize:한 화면에 보이는 레코드 수
	         result += "&nbsp;<a href='" + url  
	               + "?pageNumber=" + ( 1 ) + "&pageSize=" + this.pageSize + added_param+"'><<</a>&nbsp;" ;
	         result += "&nbsp;<a href='" + url 
	...
	public Paging( 
		...
		this.totalCount = totalCount ; 
		System.out.println("paging의 hotelnum ="+ hotelnum);
		this.totalPage = (int)Math.ceil((double)this.totalCount / this.pageSize) ;
		...
		this.pagingHtml = getPagingHtml_C(url) ;
		
	
	}
	
	private String getPagingHtml_C( String url ){ 
		System.out.println("getPagingHtml url:"+url); 
		
		String result = "" ;
		String added_param = "&hotelnum=" + hotelnum; 
